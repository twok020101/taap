...
- [x] **Dev-mode canvas stability** — replaced `cobe` entirely. Prod user report confirmed the `removeChild` throw was not dev-only; cobe's WebGL teardown collided with React 19's reconciler on `[city]` navigation in both dev and prod. The `GlobeIntro` is now a WebGL-free splash (logo + CSS pulse ring + city name + tagline, fades over ~2.6 s, same sessionStorage dedup + prefers-reduced-motion + click-to-skip behaviour) and `GlobeSpinner` is now a CSS border-rotate loader. `cobe` removed from `package.json`. Verified: navigating `/` → `/bangalore` → back → `/delhi` → `/mumbai` in the preview browser produces zero console errors and renders the intro per city.
